...
TODO: Because =irony= does not recognize 'self-added' include files (such as =Eigen=), I cannot get completions of the related codes. Fix this.

Note that =flycheck= do recognize these include files because ~flycheck-clang-include-path~ is properly configured.
#+begin_src emacs-lisp :tangle yes
  (use-package irony
...
=flycheck-irony= offers an =irony=-backend for =flycheck= mode.
The following should work, but not for my environment, and I don't have time to figure out this conundrum right now.
#+begin_src emacs-lisp :tangle yes
